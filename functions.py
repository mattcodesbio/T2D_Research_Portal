...
def load_clr_results(conn, sweed_report, population, chromosome):
    """
    Loads CLR results from the report file generated by SweeD into the database.

    The function expects the file to follow a specific format:
    ...
    The function performs the following steps:
    1. Reads the TSV file into a DataFrame, skipping the first two rows.
    2. Cleans up data by removing rows where 'Position' is not a number.
    3. Renames columns to match the table schema.
    4. Adds population and chromosome columns.
    5. Adds a column for id, assuming we want to manually assign ids starting from 1 or max(id)+1.
    6. Reorders columns to match the table schema.
    7. Inserts the data into the 'clr_results' table.
    ...
    cursor.execute('SELECT MAX(id) FROM clr_results')
    ...
    df.to_sql('clr_results', conn, if_exists='append', index=False)
    print(f"Data from '{sweed_report}' inserted successfully.")

# Connect to the SQLite database
conn = sqlite3.connect('/home/bt24998/BIO727P/T2D_Web_development_project/instance/database.db')
...
for directory in directories:
    for filename in os.listdir(directory):
        if filename.startswith('SweeD_Report'):
            ...
            load_clr_results(conn, sweed_report, population, chromosome)
