...
for pop in "${populations[@]}"; do
    ...
    mkdir -p "$pop_dir/${pop}_TajimaD_results"

    # Loop through each gene region
    for gene in "${gene_regions[@]}"; do
        echo "Processing gene region: $gene"
        vcf_file="$pop_dir/${pop}_${gene}.vcf.gz"

        if [[ -f "$vcf_file" ]]; then
            echo "Calculating Tajima's D for $pop in $gene..."
            vcftools --gzvcf "$vcf_file" --TajimaD $WINDOW_SIZE --out "$pop_dir/${pop}_TajimaD_results/${pop}_${gene}_TajimaD"
        else
